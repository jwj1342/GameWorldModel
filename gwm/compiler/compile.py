...
def compile_program(program: dict, out_dir: str | Path, cfg: dict | None = None) -> dict:
    report = validate(program)
    if not report["ok"]:
        return {"ok": False, "validation": report}
    manifest = resolve_assets(program)
    game_dir = bundle(program, out_dir, manifest, report, cfg)
...
def main(argv=None):
    ...
    res = compile_program(program, a.out_dir, load_config())
    if not res["ok"]:
